import { areFeedbackSoundsEnabled } from "@/features/question-session/hooks/useFeedbackSoundPreference";
...
function getAudioContext() {
  if (typeof window === "undefined" || !globalThis.AudioContext) return null;
...
function playAudioElement(sound: HTMLAudioElement | null) {
  ...
  if (!playback) return;
  void playback.catch(() => {
    // Browser audio restrictions should not interrupt the answer flow.
  });
}
